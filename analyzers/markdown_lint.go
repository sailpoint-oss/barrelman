...
import (
	...
	"github.com/sailpoint-oss/barrelman/markdown"
)

var (
	descriptionMarkdownMeta = barrelman.RuleMeta{
		...
		DocURL:      barrelman.DocBaseURL + "description-markdown",
	}

	descriptionHTMLMeta = barrelman.RuleMeta{
		ID:          "description-html",
		...
		DocURL:      barrelman.DocBaseURL + "description-html",
	}
...
func registerMarkdownAnalyzers(reg *barrelman.Registry) {
	barrelman.Define("description-markdown", descriptionMarkdownMeta).
	...
	barrelman.Define("description-html", descriptionHTMLMeta).
		Custom(func(idx *navigator.Index, r *barrelman.Reporter) {
			for _, desc := range collectDescriptions(idx) {
				checkHTML(desc, r)
			}
...
func collectDescriptions(idx *navigator.Index) []navigator.DescriptionValue {
	if idx == nil || idx.Document == nil {
		return nil
	}
	doc := idx.Document
	var entries []navigator.DescriptionValue

	add := func(dv navigator.DescriptionValue) {
		if dv.Text != "" {
...
func collectSchemaDescriptions(schema *navigator.Schema, entries *[]navigator.DescriptionValue) {
	if schema == nil {
		return
	}
...
func validateDescription(desc navigator.DescriptionValue, r *barrelman.Reporter) {
...
func checkHTML(desc navigator.DescriptionValue, r *barrelman.Reporter) {
