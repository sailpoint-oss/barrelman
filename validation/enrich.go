...
func (e *TypoEnricher) Enrich(err *ValidationError, _ []byte) *ValidationError {
	...
	if bestDist >= 0 && bestDist <= 2 && bestMatch != "" {
		...
		enriched.Fixes = append(enriched.Fixes, barrelman.Fix{
			Description: fmt.Sprintf("Replace with %q", bestMatch),
			Edits: []barrelman.TextEdit{{
				Range:   err.Range,
				NewText: bestMatch,
			}},
		})
		...
		enriched.Fixes = append(enriched.Fixes, barrelman.Fix{
			...
			Edits: []barrelman.TextEdit{{
				Range: barrelman.Range{
					Start: barrelman.Position{Line: insertLine, Character: 0},
					End:   barrelman.Position{Line: insertLine, Character: 0},
				},
				NewText: newText,
			}},
		})
