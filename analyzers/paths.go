...
var (
	kebabCaseMeta = barrelman.RuleMeta{
		ID:          "kebab-case",
		Description: "Path segments should use kebab-case.",
		Severity:    barrelman.SeverityWarning,
		Category:    barrelman.CategoryPaths,
		Recommended: true,
		HowToFix:    "Rename path segments to use kebab-case (lowercase with hyphens).",
		DocURL:      barrelman.DocBaseURL + "kebab-case",
	}

	noTrailingSlashMeta = barrelman.RuleMeta{
		...
		DocURL:      barrelman.DocBaseURL + "path-keys-no-trailing-slash",
	}

	noHTTPVerbsMeta = barrelman.RuleMeta{
		ID:          "no-http-verbs",
		...
		DocURL:      barrelman.DocBaseURL + "no-http-verbs",
	}

	paramsMatchMeta = barrelman.RuleMeta{
		ID:          "path-params",
		...
		DocURL:      barrelman.DocBaseURL + "path-params",
	}

	templateValidMeta = barrelman.RuleMeta{
		...
		DocURL:      barrelman.DocBaseURL + "path-declarations-must-exist",
	}

	idUniqueInPathMeta = barrelman.RuleMeta{
		...
		DocURL:      barrelman.DocBaseURL + "id-unique-in-path",
	}

	casingConsistencyMeta = barrelman.RuleMeta{
		...
		DocURL:      barrelman.DocBaseURL + "casing-consistency",
	}

	noGenericParamNamesMeta = barrelman.RuleMeta{
		...
		DocURL:      barrelman.DocBaseURL + "path-param-values-no-generic-syntax",
	}
)

var genericSyntaxRe = regexp.MustCompile(`<[^>]+>|:[a-zA-Z_]+`)

func registerPathsAnalyzers(reg *barrelman.Registry) {
	barrelman.Define("kebab-case", kebabCaseMeta).Paths(func(path string, item *navigator.PathItem, r *barrelman.Reporter) {
	...
	barrelman.Define("path-keys-no-trailing-slash", noTrailingSlashMeta).Paths(
		func(path string, item *navigator.PathItem, r *barrelman.Reporter) {
	...
	barrelman.Define("no-http-verbs", noHTTPVerbsMeta).Paths(
		func(path string, item *navigator.PathItem, r *barrelman.Reporter) {
	...
	barrelman.Define("path-params", paramsMatchMeta).Custom(
		func(idx *navigator.Index, r *barrelman.Reporter) {
			for path, item := range idx.Document.Paths {
				templateParams := barrelman.ExtractPathParams(path)
				for _, mo := range item.Operations() {
					...
					for _, tp := range templateParams {
						if !declaredParams[tp] {
							r.At(navigator.LocOrFallback(mo.Operation.ParametersLoc, mo.Operation.Loc), "Path parameter '{%s}' in %s not declared in %s operation", tp, path, mo.Method)
						}
					}
				}
			}
	...
	barrelman.Define("path-declarations-must-exist", templateValidMeta).Paths(
		func(path string, item *navigator.PathItem, r *barrelman.Reporter) {
	...
	barrelman.Define("id-unique-in-path", idUniqueInPathMeta).Paths(
		func(path string, item *navigator.PathItem, r *barrelman.Reporter) {
			params := barrelman.ExtractPathParams(path)
	...
	barrelman.Define("casing-consistency", casingConsistencyMeta).Custom(
		func(idx *navigator.Index, r *barrelman.Reporter) {
			...
			for path := range idx.Document.Paths {
				for _, seg := range barrelman.NonParamSegments(path) {
					switch {
					case barrelman.IsKebabCase(seg) && strings.Contains(seg, "-"):
						casingCounts["kebab"]++
			...
			for path, item := range idx.Document.Paths {
				for _, seg := range barrelman.NonParamSegments(path) {
					...
					switch dominant {
					case "kebab":
						mismatch = !barrelman.IsKebabCase(seg)
					case "snake":
	...
	barrelman.Define("path-param-values-no-generic-syntax", noGenericParamNamesMeta).Paths(
		func(path string, item *navigator.PathItem, r *barrelman.Reporter) {
